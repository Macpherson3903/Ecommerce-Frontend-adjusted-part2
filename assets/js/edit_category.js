...
const response = await fetch(`https://swisstools-store.onrender.com/api/categories/${categoryId}`);
...
document.querySelector(".save-btn").addEventListener("click", async function() {
  try {
    ...
    const response = await fetch("https://swisstools-store.onrender.com/api/edit_category", { method: "PUT", headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, name, description }) });
    const result = await response.json();
    if (result.success) {
